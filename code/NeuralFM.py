...
class NeuralFM(BaseEstimator, TransformerMixin):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            for i in range(0, len(self.layers)):
                ...
                if i==len(self.layers)-1:                   
                    self.deep_hidden_feature = self.FM #ZY:ADD: output feature of the hidde layers 
    ...
    def get_random_block_from_data_balanced(self, data, batch_size):  #ZYEDIT: 
    # generate a random block of training data but with some constrait
    # for each batch, try to balanced the samples (must contain positive sample and try to balanced btw pos and neg)
        
        Y_gt = data['Y']
        i_y_pos = [i for i,j in enumerate(Y_gt) if j==1]
        i_y_neg = [i for i,j in enumerate(Y_gt) if not j==1] #for different data, the negative might be labeld as 0 or -1
        
        #print("#positive = %d, #negative = %d" %(len(i_y_pos), len(i_y_neg)))
        #assert len(i_y_neg)+len(i_neg)==len(Y_gt): "get balanced batch error!"     
        batch_size_pos = min(len(i_y_pos), batch_size/2)
        batch_size_neg = min(len(i_y_neg), batch_size/2)
        
        if batch_size_pos < batch_size/2: #if there is not sufficient positive samples, make up by negatives if availiable 
            if len(i_y_neg) > batch_size/2:
                batch_size_neg = min(len(i_y_neg), max(0, batch_size - batch_size_pos))
        if batch_size_neg < batch_size/2: #if there is not sufficient negative samples, make up by positives if availiable 
            if len(i_y_pos) > batch_size/2:
                batch_size_neg = min(len(i_y_pos), max(0, batch_size - batch_size_neg))
        print("Going to select %d /%d, %d/%d" %(batch_size_pos, len(i_y_pos), batch_size_neg, len(i_y_neg)))        
        
        data_pos = {} 
        data_pos['X'] = [ data['X'][i] for i in i_y_pos] 
        data_pos['Y'] = [ data['Y'][i] for i in i_y_pos] 
        data_pos = self.get_random_block_from_data(data_pos, batch_size_pos)
        #print(type(data_pos['Y']), len(data_pos['Y']), type(data_pos['Y'][0]))
        #print(data_pos['Y'])
        
        data_neg = {} 
        data_neg['X'] = [ data['X'][i] for i in i_y_neg] 
        data_neg['Y'] = [ data['Y'][i] for i in i_y_neg] 
        data_neg = self.get_random_block_from_data(data_neg, batch_size_neg)
        #print(data_neg['Y'])
        #print("Batch: #positive = %d, #negative = %d" %(len(data_pos['Y']), len(data_neg['Y'])))
         
        
        merge = {} 
        merge['X'] = data_pos['X']+ data_neg['X']
        merge['Y'] = data_pos['Y']+ data_neg['Y']        
    ...
    def get_deep_feature(self, data): #ZYEDIT: 
        #return the feature vector for the input data
        ...
        features = self.sess.run((self.deep_hidden_feature), feed_dict=feed_dict)
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            for i in range(total_batch):
                # generate a batch
                batch_xs = self.get_random_block_from_data_balanced(Train_data, self.batch_size) #ZYEDIT: get_random_block_from_data(Train_data, self.batch_size)
                # Fit training
...
if __name__ == '__main__':
    # Data loading
    args = parse_args()
    X_train = np.load(args.path + args.dataset + '.train.X.npy')
    Y_train = np.load(args.path + args.dataset + '.train.y.npy')
    X_validation = np.load(args.path + args.dataset + '.train.X.npy')
    Y_validation = np.load(args.path + args.dataset + '.train.y.npy')
    X_test = np.load(args.path + args.dataset + '.train.X.npy')
    Y_test = np.load(args.path + args.dataset + '.train.y.npy')
    data = DATA.LoadData(args.loss_type, X_train, Y_train, X_validation, Y_validation, X_test, Y_test)
    if args.verbose > 0:
